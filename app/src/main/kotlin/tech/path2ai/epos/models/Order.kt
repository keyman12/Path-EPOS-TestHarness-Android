...
data class CompletedOrder(
     ...
     * before tipping — callers should fall back to [amountPence].
     */
    ...
    /**
     * Void = full reversal of an approved sale (no amount, no card tap). A
     * completed card sale with a terminal reference — but only before any item
     * has been refunded (a part-refunded sale can no longer be voided).
     */
    val canVoid: Boolean
        get() = status == OrderStatus.COMPLETED && orderType == OrderType.SALE &&
