...
/**
 * Open a tab: enter a name, pick a pre-auth hold amount, place the hold (a faked
 * card tap on the OCPay loopback). On approval the tab is saved and we return to
 * the Tabs list. Auto-connects if the terminal isn't connected yet.
 */
        ...
        Column(
            ...
            if (!connected) {
                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.errorContainer)) {
                    Text("Not connected to a terminal — it'll connect automatically when you place the hold.",
            ...
            Button(
                enabled = !busy && name.isNotBlank() && (preAuthPence() ?: 0) > 0,
                modifier = Modifier.fillMaxWidth(),
                onClick = {
                    ...
                    scope.launch {
                        try {
                            if (terminalManager.connectionState.value !is TerminalConnectionState.Connected) {
